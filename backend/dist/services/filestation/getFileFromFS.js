 ...
 */
export const getFileFromFS = async (fileName) => {
    const { FS_API_URL } = process.env;
    const { sessionId, synoToken } = await FileStationCredentials.getInstance();
    const url = `${FS_API_URL}/webapi/FileStation/file_thumb.cgi?api=SYNO.FileStation.Thumb` +
    ...
    const parseRes = z.instanceof(File).safeParse(res.data);
    if (!parseRes.success) {
