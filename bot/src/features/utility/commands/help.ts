...
const execute = async (interaction: ChatInputCommandInteraction) => {
  const settingCmds =
    "- `/subscribe <channel>`: ask polaroids to upload any attachments sent in a channel, or link a channel to a different album\n" +
    "- `/unsubscribe <channel>`: unsubscribe polaroids from changes in a channel\n";
  const utilCmds =
    "- `/album create | editinfo | delete`: create, edit, or delete an album\n" +
