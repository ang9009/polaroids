    ...
    async execute(message) {
        ...
        if (unsupportedAttachmentsString) {
            ...
            if (files.length === 0) {
                ...
                initialMsgRef.edit(`File(s) of unsupported formats found: ${unsupportedAttachmentsString}. These will be ignored.`);
            }
        }
        const allFilesData = getAttachmentsUploadData(files, ids, contentTypes);
 ...
 */
const getAttachmentsTotalSizeString = (files) => {
 ...
 */
const getAttachmentsUploadData = (files, ids, contentTypes) => {
